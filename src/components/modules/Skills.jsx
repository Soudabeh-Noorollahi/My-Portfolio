...
function Skills() {
  return (
    <div className="grid xl:px-16 px-6 mx-auto mb-10 pb-14">
      <h4 className="text-3xl w-fit px-3 font-bold xl:float-right mx-auto text-neutral-700 mt-8 border-b-2 pb-2 border-violet-600">
        Expertise
      </h4>
      ...
      <motion.div className="grid xl:grid-cols-6 md:grid-cols-4 grid-cols-3 mt-6 gap-6 mx-auto text-center">
        {skills.map(({ icon: Icon, label, color }, index) => (
          <motion.div
            key={label}
            className="flex flex-col items-center justify-center w-[90px] h-[90px] bg-gradient-to-t from-sky-100 rounded-md hover:border-b-2 hover:border-violet-400 transition-colors cursor-pointer"
